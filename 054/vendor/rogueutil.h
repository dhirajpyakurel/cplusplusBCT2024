 ...
 */
#ifdef __cplusplus
void setString(const RUTIL_STRING& str_) {
...
#ifdef UNICODE
    WriteConsoleOutputCharacterA(hConsoleOutput, str, len,
 ...
 * @param fmt printf-style formatted string to print in C or a list of objects
 * in C++
 * @param color Foreground color to be used, use -1 to use the currently set
 * foreground color
 * @param bgcolor Background color to be used, use -1 to use the currently set
 * background color
 * @see color_code
 */
#ifdef __cplusplus
void colorPrint(color_code, color_code) { resetColor(); }

template <typename T, typename... F>
void colorPrint(color_code color, color_code bgcolor, T arg, F... fmt) {
    if (color >= 0)
        setColor(color);
